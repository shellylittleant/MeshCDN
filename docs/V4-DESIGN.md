...
配置 snapshot 数据量小，RTT 低基本=快。程序升级（5MB binary）值得做小探针：升级时向候选 peer 发 1MB 探针 HEAD 请求，挑带宽最高者下载。（带宽探测/多源下载暂未实现，所有 peer 从发起节点拉。）

**升级可靠性（v4.3.0+）**——`/v upgrade` 从"发出即成功"改为**可观测 + 可回滚**：

- **可观测**：发起节点推送 `/mesh/upgrade` 后，**轮询各 peer 心跳的 program_version**（最多 90s），回报「N/M 节点已确认升级 + 失败清单」。此前是"先回 OK 再异步下载/替换/重启"，任何 ack 之后的失败（下载失败/sha 不符/重启失败）操作者都看不到——这正是"一键升级静默失败、需手动逐台补"的根因。权威版本状态随时可用 `/v nodes` 复核（心跳持续上报每节点版本）。
- **可回滚**：peer 替换二进制前先备份 `<bin>.old`；替换后由 `systemd-run` 拉起一个**独立瞬态单元**做看门狗（必须独立于 agent 的 KillMode=mixed cgroup，否则会被自身重启杀掉）——重启 agent，60s 内若未连续 3 次 `is-active`（崩溃回环会在此窗口内进入 failed），则 `cp .old` 回滚并再次重启。
- **注意**：回滚看门狗代码位于"接收升级的 peer 当前运行的二进制"里，故从旧版（无此逻辑）首次升级时**只有发起端的核对/回报生效，回滚从下一次升级起才有**。发起节点不重启自己（操作者需先把新二进制部署到发起节点）。
...
**多 SAN 续签（v4.0.20+）**：续签必须**保留证书完整 SAN 集**，不能缩水成 CN。scanner 把 `{Subject} ∪ SAN`（去重）作为续签标识集塞进任务；worker 用它调用 `ACMEClient.IssueMulti` / `cert.GenerateSelfSignedMulti` 重签。此前只用 `cm.Subject`（CN），会把用户多域名证书削成单域名，其余域名掉到下一优先级（可能自签，触发浏览器告警）。含 IP 的 SAN 集走空 CN 的 CSR + shortlived profile（LE 对 IP 证书的要求）。
...
- **默认**：join_order 最小（第一个安装）的节点为 bot（`peers.BotNodeIP()`）。
- **显式转移** `/v target <ip>`（v4.0.20+）：把 `cluster_meta.bot_node_ip` 覆盖写为目标 IP。这是 read-shaped 动作但**强制 +1 版本号**（`Effects.ForceVersionBump`），随配置流同步全集群；snapshot 以 `/w internal set-bot <ip>` 表示（可重放、跨升级保留）。有效 bot = 覆盖值（若设）否则 join_order 默认——这是 status/nodes 显示、告警路由、boot 门控三处共用的唯一判据。
- **生效时机（状态层）**：覆盖写入后立即全集群可见（status/nodes/告警路由随之改变）；但**目标节点实际接管 Telegram 轮询需在下次 agent 重启后**——boot 门控读覆盖值决定本节点是否轮询，且覆盖优先于安装期的 `MESHCDN_BOT_DISABLE`。免重启的实时切换是后续独立任务。
- bot 失联：**纯手动接管**。SSH 到任一存活节点执行 `/v target <本机>`（或清除覆盖 `/w internal set-bot -` 回退默认），重启后接管。
...
### 缓存清理（read-shaped 动作，v4.0.20+）

```
/v cache    -                 purge-all           清空全集群缓存 + reload
/v cache    -                 purge-node          仅清空本节点缓存（集群广播的落地形式）
```

纯动作型：不写库、不进 `config_version`、不进 snapshot（参照 `/v sync`）。`purge-all` 清本机后经 `/mesh/exec` 把 `purge-node` 广播给每个 peer，各自清自己的缓存目录并 reload；peer 收到的是**不再广播**的 `purge-node`，故无广播风暴。OpenResty 用内容哈希做 cache key，故只做全清（`rm -rf cache/*` + reload），不做按 pattern 精细清除。实现委托 executor 的 `PurgeCache` effect（handler 不碰文件系统）。
...
**实现补记（v4.0.20+，defense UA 拦截）**：`defense` 对象的 `ua=` 走 OpenResty Lua。cdn-agent 把某 server 下所有绑定 defense 对象的 UA 正则合并成**单个** `access_by_lua_block`（一次请求一个循环，不是每条规则一个 block），用 `ngx.re.match(ua, pat, "jo")` 匹配，命中 `ngx.exit(403)`。该块在 nginx `allow/deny` 之后的 access 阶段执行，天然满足"block 优先"（IP 已被 deny 的请求不会走到 UA 检查）。正则以 Lua 字符串字面量嵌入（`luaQuote` 转义），并在 `Validate()` 阶段拒绝控制字符、灾难性回溯（ReDoS）结构与非法正则——坏 pattern 永远进不到配置生成。

**B 类对象 key 为封闭集合**：任何未知 key 在解析阶段**报错**（此前静默丢弃，属静默失效）。`geo=`/`cc=` 尚未实现，显式报错而非假装成功。
...
`runtime/logs.db` 是访问统计的独立数据库（v4.0.20+ 实现，单节点范围）。**只存按分钟聚合，不存原始逐条请求**（原始日志体量会爆炸，且 runtime/ 升级即删）：

```sql
CREATE TABLE request_stats (            -- 每 (域名,分钟,状态码) 一行
    domain TEXT, minute INTEGER,        -- minute = unix epoch 截断到分钟
    status INTEGER, hits INTEGER, bytes INTEGER,
    PRIMARY KEY (domain, minute, status)
);
CREATE TABLE collector_state (id=1, log_offset INTEGER);  -- 采集断点(防重启重复计数)
```

采集链路（V4-DESIGN §A.3 的第 4 个循环）：nginx `escape=json` 的 `stats.log`（字段 `t/host/status/bytes`）→ `internal/logs` collector 每分钟 tail 增量、按分钟聚合入库、持久化 offset、清理超 7 天数据 → `/v stats` 查询。采集在 nginx 请求路径之外（非 `log_by_lua`），延迟 ≤1 分钟。跨节点聚合（§3.3）未实现，`/v stats` 数字仅本节点。
