...
# ─────────────────────────────────────────────────────────────────────
# Step 4a: Auto-build from source if no pre-built binary present.
#
# If install.sh sits in a MeshCDN source tree (i.e. ../Makefile + ../go.mod
# exist), we install Go (if absent), run `go mod tidy`, and `make build`,
# then stage the result alongside install.sh so the rest of this script
# finds it via SCRIPT_DIR/cdn-agent. This makes the user-facing flow a
# single command: `sudo bash scripts/install.sh --bot-token=... --group-id=...`
#
# Falls through silently if we're a release-tarball drop without source
# (then the existing "binary not found" error below kicks in).
# ─────────────────────────────────────────────────────────────────────
SOURCE_ROOT="$(cd "$SCRIPT_DIR/.." 2>/dev/null && pwd || echo "")"

if [ -z "$CDN_AGENT_SRC" ] && \
   [ -n "$SOURCE_ROOT" ] && \
   [ -f "$SOURCE_ROOT/Makefile" ] && \
   [ -f "$SOURCE_ROOT/go.mod" ]; then

    echo
    echo "▸ No pre-built cdn-agent found; building from source at $SOURCE_ROOT"

    # ── Go: install if missing or older than 1.21 ──────────────────────
    GO_OK=0
    if command -v go >/dev/null 2>&1; then
        GOV="$(go version 2>/dev/null | awk '{print $3}' | sed 's/^go//')"
        GO_MAJOR="$(echo "$GOV" | cut -d. -f1)"
        GO_MINOR="$(echo "$GOV" | cut -d. -f2)"
        if [ -n "$GO_MAJOR" ] && [ -n "$GO_MINOR" ]; then
            if [ "$GO_MAJOR" -gt 1 ] 2>/dev/null || \
               { [ "$GO_MAJOR" -eq 1 ] 2>/dev/null && [ "$GO_MINOR" -ge 21 ] 2>/dev/null; }; then
                GO_OK=1
            fi
        fi
    fi
    ...
    if [ "$GO_OK" -ne 1 ]; then
        ...
        case "$(uname -m)" in
            ...
            *) echo "ERROR: unsupported arch $(uname -m) for auto Go install" >&2; exit 1 ;;
        esac
        GO_TARBALL="go1.23.4.linux-${GOARCH}.tar.gz"
        TMP_GO_DIR="$(mktemp -d)"
        GO_OK_DL=0
        for src in \
        ...
        do
            echo "    trying: $src"
            if curl -fsSL --connect-timeout 8 --max-time 180 -o "$TMP_GO_DIR/$GO_TARBALL" "$src"; then
                GO_OK_DL=1; break
            fi
            echo "    (failed, trying next mirror...)"
        done
        if [ "$GO_OK_DL" -ne 1 ]; then
            echo "ERROR: could not download Go from any mirror" >&2
            exit 1
        fi
        rm -rf /usr/local/go
        ...
        # Make sure /usr/local/go/bin is in PATH for this script even if
        # the parent shell didn't source profile.d/.
    ...
    # ── Build ──────────────────────────────────────────────────────────
    # goproxy.cn is a public mirror of proxy.golang.org maintained by qiniu;
    # safe globally (just a cache). sum.golang.google.cn is Google's
    # checksum database mirror for users behind the Great Firewall.
    ...
    if [ ! -x "$SOURCE_ROOT/cdn-agent" ]; then
        echo "ERROR: build did not produce $SOURCE_ROOT/cdn-agent" >&2
        exit 1
    fi

    cp -f "$SOURCE_ROOT/cdn-agent" "$SCRIPT_DIR/cdn-agent"
    chmod +x "$SCRIPT_DIR/cdn-agent"
    CDN_AGENT_SRC="$SCRIPT_DIR/cdn-agent"
    echo "  ✓ built and staged: $CDN_AGENT_SRC"
fi
